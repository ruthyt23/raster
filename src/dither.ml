...
let%expect_test "transform" =
  let beach = Image.load_ppm ~filename:"../images/beach_portrait_gray.ppm" in
  let dither_beach = transform beach in
  let ref_dither_beach =
    Image.load_ppm ~filename:"../images/reference-beach_portrait_dither.ppm"
  in
  let pixels_wrong =
    Image.foldi dither_beach ~init:0 ~f:(fun ~x ~y acc _ ->
      if Pixel.equal
           (Image.get dither_beach ~x ~y)
           (Image.get ref_dither_beach ~x ~y)
      then acc
      else acc + 1)
  in
